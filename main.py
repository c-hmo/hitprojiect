...
def get_github_trending(since):
    """抓取 GitHub Trending (获取当前页面所有项目，官方默认最大 25 个)"""
    url = f"https://github.com/trending?since={since}"
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
    
    print(f"开始抓取榜单: {since}...")
    repos = []
    try:
...
def build_html_email(hot_repos, surging_repos):
    ...
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; padding: 20px; color: #333;">
        <h2 style="color: #d73a49; border-bottom: 2px solid #d73a49; padding-bottom: 5px;">🔥 本周热门项目 (Top {len(hot_repos)})</h2>
        {build_cards(hot_repos, "#d73a49", "🔥 热门连续")}
        
        <h2 style="color: #28a745; border-bottom: 2px solid #28a745; padding-bottom: 5px; margin-top:30px;">🚀 24小时飙升项目 (Top {len(surging_repos)})</h2>
        {build_cards(surging_repos, "#28a745", "🚀 飙升连续")}
        
        <div style="text-align: center; margin-top: 30px; font-size: 12px; color: #888;">
            由 GitHub Actions 与 Gemini API 强力驱动 | 智能去重与批处理架构已激活
        </div>
    </body>
    </html>
    """
...
if __name__ == "__main__":
    ...
    # 获取官方页面提供的全部项目数据 (默认每项最高 25 个)
    weekly_repos_all = get_github_trending("weekly")
    daily_repos_all = get_github_trending("daily")
    
    # 1. 选取 Top 10 热门项目
    hot_repos = weekly_repos_all[:10]
    
    # 建立热门项目名称集合，用于后续去重
    hot_repo_names = {repo['name'] for repo in hot_repos}
    
    # 2. 选取飙升项目 (智能过滤掉已经在热门榜里的项目，目标 20 个)
    surging_repos = []
    for repo in daily_repos_all:
        if repo['name'] not in hot_repo_names:
            surging_repos.append(repo)
        if len(surging_repos) == 20:
            break
            
    print(f"抓取与去重完毕: 热门 {len(hot_repos)} 个, 飙升 {len(surging_repos)} 个 (共需处理 {len(hot_repos) + len(surging_repos)} 个)")
